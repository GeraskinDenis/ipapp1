...
//ТНК
public class Activity_norm extends EntityDecorator {

	private static final String TABLE_NAME = "activity_norm";

	public Activity_norm() {
		this.record = new EntityDTO(TABLE_NAME);
	}

	public Activity_norm(EntityDTO record) {
		if (record == null)
	...
	//ID
	public Long get_Activity_normid() {
		return record.getAsLong("activity_normid");
	}
	...
	public Double get_Measure() {
		return record.getAsDouble("measure");
	}
	...
	//ID
	public void set_Activity_normid(Long fieldValue) {
		record.set("activity_normid", fieldValue);
	}
	...
	//Измеритель
	public void set_Measure(Double fieldValue) {
		record.set("measure", fieldValue);
	}
	...
	//Единицы измерения
	public void set_Unit_of_measure(Long fieldValue) {
		record.set("unit_of_measure", fieldValue);
	}
