...
Fields:
 ...
 - software (ПО для рабочего места) - INTEGER
 ...
 - workplaceid (ID) - INTEGER
 - workspace (Рабочее пространство) - STRING
*/

public class workplace_Rules {
	...
	public static void afterUpdate(EntityDTO record, EntityDTO oldrecord) throws Exception {
		...
		//region Update Owner of Related Objects
		//rule <sn>611ddfaa-2b38-2512-8627-c31b84d49762</sn>
		if (/*if*/!CommonUtils.isSame(record.get("owner"),oldrecord.get("owner"))/*if*/) {
			//<body>
			List<EntityDTO> relList = QueryUtils.getRecordList("cirelation","citype1="+record.get("citype")+" and ci1="+record.getId());
			for (EntityDTO rel:relList){
			    String tableName =rel.getAsString("citype2->entitytype->tablename");
			    if (!CommonUtils.isSame(tableName, "software") && (!CommonUtils.isEmpty(tableName))){
			        EntityDTO ci2 = QueryUtils.getRecordById(tableName, rel.getAsInteger("ci2"));
			        if (!CommonUtils.isEmpty(ci2)){
			            ci2.set("owner", record.getAsInteger("owner"));
			            ci2.doUpdate();
			        }
			    }
			}
