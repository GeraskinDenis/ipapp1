...
public class monitor_Rules {
	...
	public static void afterInsert(EntityDTO record, EntityDTO oldrecord) throws Exception {
		...
		//region Create RSM Relation monitor with workspace after integration
		//rule <sn>fd8334ee-229d-3414-8a4c-4e47b834e6e8</sn>
		if (/*if*/!record.isFieldEmpty("workplace") && !CommonUtils.isSame(record.getAsInteger("workplace"),oldrecord.getAsInteger("workplace"))/*if*/) {
			//<body>
			EntityDTO workplace = QueryUtils.getRecordById("workplace", record.getAsInteger("workplace"));
			List <Integer> list = workplace.getIntegerArray("monitors");
			if (!list.contains(record.getId())) {
			list.add(record.getId());
			workplace.set("monitors", list);
			workplace.doUpdate();
			}
			//</body>
		}
	...
	public static void beforeUpdate(EntityDTO record, EntityDTO oldrecord) throws Exception {
		...
		//region Update QR-code data
		//rule <sn>02c24761-9a62-e566-aec1-5090880f4121</sn>
		if (/*if*/!CommonUtils.isEmpty(record.get("serialno"))/*if*/) {
			//<body>
			record.set("qr_code", record.getAsString("serialno"));
			//</body>
		}
		//endregion
	...
	public static void beforeDelete(EntityDTO record, EntityDTO oldrecord) throws Exception {
		...
		//region Delete RSM Relation monitor with workspace after integration
		//rule <sn>ed8fccd6-343c-5db4-218a-67c379ef3793</sn>
