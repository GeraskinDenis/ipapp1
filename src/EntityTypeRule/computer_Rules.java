...
public class computer_Rules {
	...
	public static void afterInsert(EntityDTO record, EntityDTO oldrecord) throws Exception {
		...
		//region Create RSM Relation computer with workspace after integration
		//rule <sn>6ac8a410-5853-b860-d2c7-94adeb54549e</sn>
		if (/*if*/!record.isFieldEmpty("workplace")/*if*/) {
			//<body>
			EntityDTO workplace = QueryUtils.getRecordById("workplace", record.getAsInteger("workplace"));
			List <Integer> list = workplace.getIntegerArray("computers");
			if (!list.contains(record.getId())) {
			list.add(record.getId());
			workplace.set("computers", list);
			workplace.doUpdate();
			}
			//</body>
		}
	...
	public static void beforeDelete(EntityDTO record, EntityDTO oldrecord) throws Exception {
		...
		//region Delete RSM Relation computer with workspace after integration
		//rule <sn>e1f4e600-2db0-9a59-344e-55d9d45e8edd</sn>
		if (/*if*/!record.isFieldEmpty("workplace")/*if*/) {
			//<body>
			EntityDTO workplace = QueryUtils.getRecordById("workplace", record.getAsInteger("workplace"));
			List <Integer> list = workplace.getIntegerArray("computers");
